...
const findCommonItemTypeInGroup = (rucksackGroup: string[]): string | undefined => {
  ...
  return [...rucksack3].find(itemType => rucksack1Set.has(itemType) && rucksack2Set.has(itemType));
};
...
const solveDay3 = () => {
  ...
  const sumOfCommonItemsPerRucksack = rucksacks.map(findCommonItemType).map(getPriorityOfItemType).reduce(sumReducer);
  ...
  const sumOfCommonItemsPerRucksackGroup = groupsOfThree.map(findCommonItemTypeInGroup).map(getPriorityOfItemType).reduce(sumReducer);
